...
const ScheduleForm = () => {
  ...
  useEffect(() => {
    ...
    if (form.role_id === "5") {
      ...
      setBatches([]); // Trainer/Course Coordinator → clear
    }
  ...
  const handleAdd = async () => {
    try {
      ...
      Object.keys(payload).forEach((key) => {
        if (payload[key] === "" || (key === "batch_id" && form.role_id !== "5")) {
          payload[key] = null;
        }
      });

      const res = await axios.post(
        "http://localhost:4000/schedulefeedback/register",
        payload,
        { headers: { token } }
      );
    ...
    <div style={styles.container}>
      ...
      <div style={styles.row}>
        ...
        <select
          ...
          value={form.batch_id}
          onChange={handleChange}
          disabled={form.role_id !== "5"} // Enabled only for Lab Mentor
          style={styles.select}
        >

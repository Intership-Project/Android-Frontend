...
// ✅ Role mappings according to updated table
const getRoleNameById = (id) => ({ 4: 'Trainer', 5: 'Lab Mentor', 6: 'Course Coordinator' }[id] || 'Unknown');
const getRoleIdByName = (name) => ({ 'Trainer': 4, 'Lab Mentor': 5, 'Course Coordinator': 6 }[name] || 4);
...
export default function Faculty() {
  ...
  const [modalData, setModalData] = useState({ facultyname: '', email: '', password: '', role_id: 4, course_id: '' });
  ...
  const loadFaculty = async () => {
    try {
      ...
      // ✅ Ensure we map backend roles to correct names using getRoleNameById if backend gives role_id
      const formattedData = (response.data || []).map(f => ({
        ...f,
        rolename: f.role_id ? getRoleNameById(f.role_id) : f.rolename
      }));
  ...
  const openAddModal = () => {
    setModalType('add');
    setModalData({ facultyname: '', email: '', password: '', role_id: 4, course_id: '' });
    setShowModal(true);
  };
  ...
  const handleModalSave = async (e) => {
    ...
    const payload = {
      ...
      course_id: modalData.role_id === 6 // ✅ FIX: Only required for Course Coordinator
  ...
  // ✅ Use consistent rolename from mapping for counts
            ...
            <form onSubmit={handleModalSave}>
              ...
              <div style={{ marginBottom:'10px' }}>
                ...
                <select value={modalData.role_id} onChange={e=>setModalData({...modalData, role_id:Number(e.target.value)})} style={{ width:'100%', padding:'8px', borderRadius:'5px', border:'1px solid #ccc' }}>
                  ...
                  <option value={5}>Lab Mentor</option>
                  <option value={6}>Course Coordinator</option>
                </select>
              </div>
              {modalData.role_id===6 && ( // ✅ FIX: Now course dropdown appears only for Course Coordinator
                <div style={{ marginBottom:'10px' }}>
                  <label>Course:</label>
